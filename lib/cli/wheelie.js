 ...
 */
Wheelie.prototype.getDest = function() {
  return this.options.production ? this.options.dist : this.options.build;
};
...
Wheelie.prototype.build = function() {
  ...
  this.options.dest = this.getDest();
  ...
  for (var i = 0, task; (task = tasks[i]); i++) {
    ...
    var callback = task.run(this.gulp, config, options);
